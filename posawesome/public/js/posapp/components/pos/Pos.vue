<template>
  <div fluid class="mt-2">
    ...
    <v-row v-show="!dialog">
      <v-col
        v-show="!payment && !offers && !coupons && !membership"
        xl="5"
        lg="5"
        md="5"
        sm="5"
...
export default {
  ...
  methods: {
    ...
    send_customer_name_to_member(customer) {
      evntBus.$emit("mem_send_cust_name", customer);
    },
  },
  ...
  mounted: function () {
    this.$nextTick(function () {
      ...
      evntBus.$on("show_payment", (data) => {
        ...
        this.membership = false ? data === "true" : false;
      });
      ...
      evntBus.$on("show_membership_card", (data, customer) => {
        this.send_customer_name_to_member(customer);
        this.membership = true ? data === "true" : false;
        this.coupons = false ? data === "true" : false;
        this.offers = false ? data === "true" : false;
        this.payment = false ? data === "true" : false;
      });
  ...
  beforeDestroy() {
    ...
    evntBus.$off("show_membership_card");
